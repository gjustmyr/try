...
@Component({
  ...
  template: `
          ...
          <div class="orders-table-wrapper" *ngIf="!loading && filteredOrders.length > 0">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="tracking-cell">
                    <span class="tracking-number" *ngIf="order.trackingNumber">{{
                  ...
                  <td class="eta-cell">
                    ...
                    <span class="eta-badge delivered" *ngIf="order.status === 'delivered'"
                      ><i class="pi pi-check-circle"></i> Delivered</span
                    >
                  ...
                  <td class="actions-cell" (click)="$event.stopPropagation()">
                    ...
                    >
                    <span
                      class="hub-status-label transit"
                      *ngIf="order.status === 'out_for_delivery'"
                      ><i class="pi pi-send"></i> Out for Delivery</span
                    >
                    <span class="hub-status-label done" *ngIf="order.status === 'delivered'"
                      ><i class="pi pi-check"></i> Delivered</span
                    >
                  </td>
                </tr>
              </tbody>
            ...
            <div class="order-detail-panel" *ngIf="expandedOrder">
              ...
              <div class="detail-tracking-bar" *ngIf="expandedOrder.trackingNumber">
                ...
                <span
                  >Tracking Number: <strong>{{ expandedOrder.trackingNumber }}</strong></span
                >
              </div>

              <div
                class="detail-eta-bar"
                *ngIf="
                  expandedOrder.estimatedDelivery &&
              ...
              >
                ...
                <span
                  >Estimated Delivery:
                  <strong>{{
                    expandedOrder.estimatedDelivery | date: 'MMMM d, yyyy'
                  }}</strong></span
                >
                <span class="eta-countdown">{{
                  getDaysUntil(expandedOrder.estimatedDelivery)
                }}</span>
              </div>
              ...
              <div class="detail-grid">
                ...
                <div class="detail-section">
                  ...
                  <p>
                    <strong>{{ expandedOrder.user?.fullName }}</strong>
                  </p>
                ...
                <div class="detail-section" *ngIf="expandedOrder.address">
                  ...
                  <p>
                    <strong>{{ expandedOrder.address.fullName }}</strong> ·
                    {{ expandedOrder.address.phone }}
                  </p>
                  <p>
                    {{ expandedOrder.address.streetAddress }}, {{ expandedOrder.address.barangay }},
                    {{ expandedOrder.address.city }}, {{ expandedOrder.address.province }}
                  </p>
                </div>
              ...
              <div class="detail-items">
                ...
                <div class="d-item" *ngFor="let item of expandedOrder.items">
                  <div class="d-item-img">
                    ...
                    <div *ngIf="!item.productImage?.url" class="d-item-placeholder">
              ...
              <div class="detail-footer">
                <div class="detail-total">
                  Your Total:
                  <strong>₱{{ getSellerTotal(expandedOrder) | number: '1.2-2' }}</strong>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="modal-overlay" *ngIf="showConfirmModal">
      <div class="modal-box">
        <div
          class="modal-icon"
          [attr.data-type]="pendingNewStatus === 'cancelled' ? 'danger' : 'info'"
        >
          <i
            [class]="
              pendingNewStatus === 'cancelled' ? 'pi pi-exclamation-triangle' : 'pi pi-refresh'
            "
          ></i>
        </div>
        <h3>{{ pendingNewStatus === 'cancelled' ? 'Cancel Order?' : 'Update Status' }}</h3>
        <p>
          Change order <strong>{{ pendingOrder?.orderNumber }}</strong> status from
          <span class="status-chip-inline" [attr.data-status]="pendingOrder?.status">{{
            pendingOrder?.status | titlecase
          }}</span>
          to
          <span class="status-chip-inline" [attr.data-status]="pendingNewStatus">{{
            pendingNewStatus | titlecase
          }}</span>
        </p>
        <p class="sub-text" *ngIf="pendingNewStatus === 'cancelled'">
          This will return the item stocks back to inventory.
        </p>
        <div class="modal-actions">
          <button class="modal-btn secondary" (click)="cancelStatusChange()">Cancel</button>
          <button
            class="modal-btn"
            [class.danger]="pendingNewStatus === 'cancelled'"
            [class.primary]="pendingNewStatus !== 'cancelled'"
  ...
  styles: [
    `
      ...
      .status-tabs button.active .tab-badge {
      ...
      .modal-icon[data-type='info'] i {
        color: #3b82f6;
      }
      .modal-icon[data-type='danger'] i {
        color: #ef4444;
      }
      .modal-box h3 {
        font-size: 22px;
      ...
      .status-chip-inline[data-status='pending'] {
        background: #fef3c7;
        color: #92400e;
      }
      .status-chip-inline[data-status='confirmed'] {
        background: #dbeafe;
        color: #1e40af;
      }
      .status-chip-inline[data-status='processing'] {
        background: #e0e7ff;
        color: #3730a3;
      }
      .status-chip-inline[data-status='shipped'] {
        background: #fce7f3;
        color: #9d174d;
      }
      .status-chip-inline[data-status='pending_drop_off'] {
        background: #fff7ed;
        color: #c2410c;
      }
      .status-chip-inline[data-status='out_for_delivery'] {
        background: #f0fdf4;
        color: #15803d;
      }
      .status-chip-inline[data-status='delivered'] {
        background: #dcfce7;
        color: #166534;
      }
      .status-chip-inline[data-status='cancelled'] {
        background: #fee2e2;
        color: #991b1b;
      }
      .modal-actions {
      ...
      .hub-status-label.shipped {
        background: #fce7f3;
        color: #9d174d;
      }
      .hub-status-label.transit {
        background: #f0fdf4;
        color: #15803d;
      }
      .hub-status-label.done {
        background: #dcfce7;
        color: #166534;
      }
      .hub-status-label i {
      ...
      .detail-tracking-bar i {
        font-size: 18px;
        color: #7c3aed;
      }
      .detail-tracking-bar strong {
        color: #4c1d95;
        font-family: monospace;
