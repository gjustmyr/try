const { Delivery, Driver, Order, OrderItem, Address, User, DeliveryHub } = require("../models");
...
exports.assignDelivery = async (req, res) => {
  ...
  try {
    ...
    if (!orderId || !driverId || !hubId) {
      return res.status(400).json({ success: false, message: "orderId, driverId, and hubId are required" });
    }
    ...
    if (existingDelivery) return res.status(400).json({ success: false, message: "Order already has a delivery assignment" });
    ...
    const distanceKm = haversineDistance(hub.latitude, hub.longitude, destLat, destLng);
...
exports.getTracking = async (req, res) => {
  try {
    ...
    else return res.status(400).json({ success: false, message: "trackingNumber or orderId required" });
      ...
      include: [
        {
          ...
          attributes: ["id", "orderNumber", "status", "estimatedDelivery", "createdAt"],
          include: [{ model: Address, as: "address", attributes: ["fullName", "phone", "streetAddress", "barangay", "city", "province", "latitude", "longitude"] }],
        },
    ...
    if (!delivery) return res.status(404).json({ success: false, message: "Tracking info not found" });

    res.json({ success: true, data: delivery });
  } catch (error) {
    console.error("Get tracking error:", error);
    res.status(500).json({ success: false, message: "Failed to get tracking info" });
  }
};
