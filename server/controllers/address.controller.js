...
exports.addAddress = async (req, res) => {
  try {
    const { label, fullName, phone, region, province, city, barangay, postalCode, streetAddress, isDefault, latitude, longitude } = req.body;

    if (!fullName || !phone || !region || !province || !city || !barangay || !postalCode || !streetAddress) {
...
exports.deleteAddress = async (req, res) => {
  try {
    ...
    if (wasDefault) {
      const next = await Address.findOne({ where: { userId: req.user.id }, order: [["createdAt", "ASC"]] });
      if (next) {
