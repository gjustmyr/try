  ...
  try {
      ...
      include: [
        {
          model: User,
          as: "user",
          attributes: ["id", "email", "isActive", "createdAt", "lastLogin"],
        },
      ],
    });
    if (!seller)
    ...
    if (orderIds.length > 0) {
      const orders = await Order.findAll({
        where: { id: { [Op.in]: orderIds }, status: { [Op.ne]: "cancelled" } },
      });
      // Sum seller items only
      for (const o of orders) {
        const items = await OrderItem.findAll({
          where: { orderId: o.id, sellerId },
        });
    ...
    res.json({
      ...
      data: {
        ...seller.toJSON(),
        stats: { productCount, totalOrders, totalRevenue },
      },
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to get seller detail" });
  }
};
...
exports.toggleSellerStatus = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Seller ${user.isActive ? "activated" : "deactivated"}`,
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to toggle seller status" });
  }
};
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Name, address, city, province, latitude, longitude, email, and password are required",
      });
    }
    ...
    const hubUser = await User.create(
      {
        ...
        password,
        userType: "hub",
        emailVerified: true, // Auto-verify hub accounts
        isActive: true,
      },
      { transaction: t },
    );
    ...
    res.status(201).json({
      success: true,
      message: "Hub and account created successfully",
      data: { hub, email },
    });
  } catch (error) {
    await t.rollback();
...
exports.deleteHub = async (req, res) => {
  try {
    ...
    if (driverCount > 0) {
      return res.status(400).json({
        success: false,
        message: "Cannot delete hub with assigned drivers",
      });
    }
    await hub.destroy();
  ...
  try {
    ...
    if (hubId) {
      ...
      if (!hub)
        return res
          .status(404)
          .json({ success: false, message: "Hub not found" });
    }
    ...
    res.status(201).json({
      success: true,
      data: { ...driver.toJSON(), user: { id: user.id, email: user.email } },
    });
  } catch (error) {
    await t.rollback();
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to create driver" });
  }
};
...
exports.getDashboardStats = async (req, res) => {
  try {
    const [
      ...
      Delivery.count({
        where: { status: { [Op.notIn]: ["delivered", "failed"] } },
      }),
    ...
    const recentOrders = await Order.findAll({
      include: [
        { model: User, as: "user", attributes: ["id", "fullName", "email"] },
        {
          ...
          as: "delivery",
          include: [
            { model: Driver, as: "driver", attributes: ["id", "fullName"] },
          ],
        },
      ],
      order: [["createdAt", "DESC"]],
...
// ============ SEARCH ORDERS (for assign delivery) ============
...
exports.searchOrders = async (req, res) => {
  try {
    ...
    const orders = await Order.findAll({
      where: {
        [Op.or]: [{ orderNumber: { [Op.iLike]: `%${q.trim()}%` } }],
      },
      include: [
        { model: User, as: "user", attributes: ["id", "fullName", "email"] },
