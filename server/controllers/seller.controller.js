...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    // Product count
    const productCount = await Product.count({ where: { sellerId: seller.id } });

    // Order stats from OrderItems belonging to this seller
    const sellerItems = await OrderItem.findAll({
      where: { sellerId: seller.id },
      include: [{ model: Order, as: "order", attributes: ["id", "status", "createdAt", "userId"] }],
    });

    const orderIds = [...new Set(sellerItems.map(i => i.order?.id).filter(Boolean))];
    ...
    // Unique customers
    const uniqueCustomers = [...new Set(sellerItems.map(i => i.order?.userId).filter(Boolean))].length;
...
exports.getSellerCustomers = async (req, res) => {
  try {
    ...
    for (const item of sellerItems) {
      ...
      if (!c.lastOrderDate || new Date(item.order.createdAt) > new Date(c.lastOrderDate)) {
